...
/** Barra horizontal CSS-only para charts */
  ...
  return (
    <div className="flex items-center gap-3 group">
      ...
      <div className="flex-1 h-6 rounded-md bg-zinc-800/60 overflow-hidden relative">
        ...
        />
        <span className="absolute inset-0 flex items-center px-2 text-[11px] font-medium text-zinc-300">
          {value}
        </span>
      </div>
...
export default async function DashboardsPage({ params }: PageProps) {
  ...
  // Fetch all tasks visible to this user across the org
  ...
  const done    = tasks.filter((t: any) => t.custom_statuses?.is_closed).length
  const open    = total - done
  const urgent  = tasks.filter((t: any) => t.priority === 'urgent' || t.priority === 'high').length
  const now     = new Date()
  ...
  const dueWeek = tasks.filter((t: any) => {
    if (!t.due_date || t.custom_statuses?.is_closed) return false
  ...
  // Tasks created last 7 days (sparkline data)
      ...
      <div className="max-w-6xl mx-auto px-8 py-8 space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500">Visão analítica de todo o workspace</p>
          </div>
          <span className="text-xs text-zinc-600 bg-zinc-800/50 px-2 py-0.5 rounded-full">
            {total} tarefa{total !== 1 ? 's' : ''} indexada{total !== 1 ? 's' : ''}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-6">
            ...
            {statusRows.length === 0 ? (
              ...
              <div className="space-y-3">
                {statusRows.map((r) => (
                  <HBar key={r.name} label={r.name} value={r.count} max={maxStatus} color={r.color} />
                ))}
              </div>
            )}
          </div>
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-6">
            ...
            {assigneeRows.length === 0 ? (
              ...
              <div className="space-y-3">
                {assigneeRows.map((r) => (
                  <HBar key={r.uid} label={r.name} value={r.count} max={maxAssignee} color="#6366f1" />
                ))}
              </div>
            )}
          </div>
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <div className="rounded-lg bg-zinc-800/50 p-3">
                <p className="text-xl font-bold text-zinc-100">{open}</p>
                <p className="text-[11px] text-zinc-500 mt-0.5">Em aberto</p>
              </div>
              <div className="rounded-lg bg-zinc-800/50 p-3">
                <p className="text-xl font-bold text-amber-400">{dueWeek}</p>
                <p className="text-[11px] text-zinc-500 mt-0.5">Vence nos próx. 7 dias</p>
              </div>
              <div className="rounded-lg bg-zinc-800/50 p-3">
                <p className="text-xl font-bold text-red-400">{overdue}</p>
                <p className="text-[11px] text-zinc-500 mt-0.5">Atrasadas</p>
              </div>
              <div className="rounded-lg bg-zinc-800/50 p-3">
                <p className="text-xl font-bold text-emerald-400">{done}</p>
                <p className="text-[11px] text-zinc-500 mt-0.5">Concluídas</p>
              </div>
            </div>

            {/* Mini sparkline — criadas nos últimos 7 dias */}
            <div>
              <p className="text-[10px] text-zinc-600 mb-2 flex items-center gap-1">
                <Calendar className="h-3 w-3" />
                Tarefas criadas (7 dias)
              </p>
              ...
              {(tasks as any[])
                ...
                .map((t) => {
                  ...
                  const assignees = (t.task_assignees ?? [])
                    .map((a: any) => memberMap.get(a.user_id))
                    .filter(Boolean)
                  const daysLeft = Math.ceil(
                    (new Date(t.due_date).getTime() - now.getTime()) / 86400000
                  )
                  return (
